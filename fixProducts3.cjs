...
prodContent = prodContent.replace(/const categoryTitle = urlCategory \? urlCategory\.toUpperCase\(\) : 'ALL PRODUCTS';/, "const categoryTitle = (searchQuery && searchQuery.trim().length > 0) ? `SEARCH: ${searchQuery}` : (urlCategory ? urlCategory.toUpperCase() : 'ALL PRODUCTS');");
...
prodContent = prodContent.replace(oldFallbackBlock, ""); 
